...
## Full-project review — 2026-07-02 (branch `chore/full-review-fixes`) — v0.5.1

Requested full review of the repo. Three Explore agents (runtime / tests+CI / docs) died to a session limit mid-audit; re-ran as direct inline greps + line-level reads, so every finding below was verified by hand. **Clean bill on**: anti-pattern greps all zero (legacy `Input.*`, `dynamic`, C#10 syntax, `Subscribe` without `using R3;`, empty catch, `async void`, TODO/FIXME), Destroy/DDOL guards complete, per-frame alloc clean (`FpsOverlay` rebuilds its string on a refresh interval; `SwipeInfo` is a readonly struct), asmdef peer rules hold, 101 EditMode tests healthy (no async-`[Test]` bug, no zero-assert files), `docs/services/*.md` format complete ×22, v0.5.0 version-sync + tag + release all consistent. Four fix groups shipped:

- **A — AdPlacement boundary guards (the one real runtime bug found).** `DefaultAdPlacementService` (the registered default binding) threw `ArgumentNullException` out of `Dictionary.TryGetValue` for null placement ids in `CanShow`/`TryShowAsync`/`NotifyShown` — contradicting both `docs/services/adplacement.md` ("fail-safe queries, explicit results") and the CLAUDE.md boundary rule — and `RegisterPlacement` accepted empty ids, `sessionCap 0`, negative cooldowns. RED-first `AdPlacementServiceTests` (15 methods; the 8 guard cases fail on the old code, incl. asserting `ParamName == "placementId"` rather than the dictionary's internal `"key"`), then the guards: queries fail safe (`false` / `Failed` result / no-op), the write method throws. `Zero.Tests.EditMode.asmdef` gains the `Zero.Services.AdPlacement` ref (service had zero tests — the only service with real logic that had none). Same doc file also had a stale "consumer partial" seam mention → rewritten to the `OnRootContainerBuilding` recipe.
- **B — constitution/doc drift.** `CLAUDE.md`: stale `6.0.3.11f1` → `6000.5.0f1` (6.5-upgrade leftover), CI description claimed "EditMode + PlayMode" but `tests.yml` is editmode-only (the PlayMode suite is a deliberate empty placeholder per PLAN §2.13 — wording now says so), and the anti-pattern list still pointed at `UnityPoolService.SafeDestroy` (moved to `Zero.Infrastructure.Util.SafeDestroy` in the 2026-05-18 de-dup). `docs/architecture/asmdef-graph.md`: still documented the inert `R3` references that v0.3.0 round C removed (×4 sections), claimed `BootstrapPipeline` lives in `Zero.Infrastructure` (it lives in `Zero.Bootstrap`), said "two services reference `Zero.Services.Events`" (one: VersionCheck), and its verify-grep used the pre-UPM `Assets/_Project/Scripts/` path.
- **C — CI cost.** Both workflows triggered on `push` (all branches) + `pull_request`, double-running every Unity job per PR push (two license activations + two full runs). Now `push: branches: [main]` + a concurrency group with cancel-in-progress. Job names unchanged → `main-required-checks` ruleset unaffected.
- **D — chore.** Inert `"R3"` swept from the 3 asmdefs round C missed (`Zero.Editor`, `Zero.Tests.EditMode`, `Zero.Tests.PlayMode` — same-shape sweep completed); `ConsoleCommandAttribute` sealed (no subclasses existed).
- **Stale resume hint above, closed:** PR-2 had in fact already shipped *before* the 6.5 upgrade — PR #7 (`e9435fc`) added the version-pin guard + consumer-mirror guard, and the `main-required-checks` ruleset has been active since 2026-06-12. PR #8 (`/version-sync` skill) likewise predates that entry.
- **Deferred (proposed during review, not done):** (1) guard sweep across the 9 mock services that accept strings unguarded (`MockAnalytics`, `MockCrashlytics`, `MockRemoteConfig`, `MockIap`, `MockNotification`, `MockConsent`, `MockAudio`, `MockAttribution`, `StubReceiptValidator`) — behavior change, wants its own branch; (2) a PlayMode boot smoke test + CI playmode job to automate the recurring "Editor verification pending" step — needs a design pass (Addressables content build in CI).
- Pre-PR gate: `asmdef-boundary-reviewer` PASS · `pitfalls-guard` PASS (its one P2 — `PITFALLS.md` still pointing at the moved `UnityPoolService.SafeDestroy` — fixed in-branch) · fresh-eyes max-effort code review: 4 Low, all applied (test-suite header no longer overstates "writes throw" vs the `NotifyShown` no-op; empty-string cases added to the `TryShowAsync`/`NotifyShown` guards; filler-`AdType` comment on the null-id `Failed` result; `CanShow("")` acknowledged as a green pin, not RED).
- Verification: static greps green (no unguarded `TryGetValue` path in AdPlacement, no `"R3"` left in any asmdef, no `6.0.3` in CLAUDE.md). **Editor verification pending — Claude cannot open Unity; user to run Test Runner EditMode (existing 101 + the new AdPlacement suite green).** Tag `v0.5.1` + GitHub release after merge.

---

## GUID collision hotfix — 2026-07-26 — v0.5.2

Found while debugging a hard compile failure in the consumer project `go_chess` (Unity 6000.5.5f1): `Zero.UI` failed with three `CS0246`s — `PopupHandle<>` (×2, `PopupBase.cs:30,53`) and `IPopupHandle` (`UIService.cs:30`) — even though `Runtime/UI/PopupHandle.cs` was present and correct on disk.

**Root cause** (Unity states it directly, `Logs/Editor.log`): `Runtime/UI/PopupHandle.cs.meta` shipped the hand-authored GUID `c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7`, which is byte-identical to the GUID in `com.unity.addressables@3.0.0/Tests/Editor/ProjectConfigDataSerializationTests.cs.meta`. Unity resolved the conflict by dropping `PopupHandle.cs` from the AssetDatabase, so the type vanished at compile time while the file stayed on disk — a maximally confusing symptom. Verified from upstream tarballs (not local PackageCache corruption): the GUID is genuinely in Addressables 3.0.0 and genuinely in Zero 0.5.0/0.5.1. Addressables 2.3.1 has no such test file, which is why this only fired on the consumer's Addressables 3.0.0 upgrade.

- **Fix**: regenerated both hand-authored GUIDs in `Runtime/UI/` — `PopupHandle.cs.meta` → `2ff6b28878ef418ab4ca669eca940c11`, `LocalizedText.cs.meta` → `a948790df0da4270bc0739e4a3155cc7` (the latter had not collided yet but carried identical risk). A full 234-meta sweep of the package found no other patterned GUIDs, and a cross-scan against all 14,201 `.meta` GUIDs in the consumer's `Assets/` + `Packages/` + `Library/PackageCache/` found no remaining collision and no collision for the two new GUIDs. No `.prefab`/`.unity`/`.asset` in this repo or in `go_chess` referenced either old GUID, so no reference rewiring was needed.
- **Files**: `Runtime/UI/PopupHandle.cs.meta` · `Runtime/UI/LocalizedText.cs.meta` · `package.json` 0.5.1→0.5.2 · `README.md` latest-pin · `CHANGELOG.md` ×2.
- **Lesson for future package work**: never hand-author `.meta` GUIDs. They look tidy (`c2d3e4f5…`) but every other author reaching for a tidy pattern lands on the same handful of strings — and Unity's failure mode is a phantom missing type, not a GUID error at the point of use.
- Verification (in consumer `go_chess`, package re-pointed at this repo via `file:` while waiting on the OpenUPM publish): `Logs/Editor.log` shows the package re-resolving to `source: local`, then a full compile with **no `CS0246` and no `GUID […] conflicts with` line** after it; `Library/ScriptAssemblies/Zero.UI.dll` now contains the `PopupHandle` / `IPopupHandle` symbols, which is the direct proof the file made it into the AssetDatabase this time. **EditMode tests not run** — the Unity MCP CLI was unavailable (plugin auto-bumped 0.81.1 → 0.86.1 mid-session and the new CLI requires an `ai-game.dev` login); user to run Test Runner EditMode before tagging. Tag `v0.5.2` + GitHub release for OpenUPM pickup.

---

## Editor patch bump — 2026-07-26 — 6000.5.0f1 → 6000.5.5f1

Environment drift found in the working tree alongside the v0.5.2 hotfix, not a deliberate upgrade: Unity had already re-stamped `ProjectVersion.txt` and rewritten `EditorBuildSettings.asset` (dropped `m_UseUCBPForAssetBundles`), and Package Manager had moved `com.unity.purchasing` 5.3.1→5.4.2 (`com.unity.services.core` 1.14.0→1.18.0 transitively). Adopted rather than reverted — `6000.5.0f1` is no longer installed on the dev machine (`/Applications/Unity/Hub/Editor/` has only `6000.5.5f1`), so a revert would re-drift on the next Editor open.
...
- Verification: static only — no live `6000.5.0f1` reference remains outside historical CHANGELOG/JOURNAL prose. **Editor verification pending.** Purchasing 5.3→5.4 is a minor bump against a template shipping an IAP service; a green CI EditMode run is strong evidence it compiles, but Play-mode IAP behavior is unverified.
