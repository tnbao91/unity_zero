...
- Verification (in consumer `go_chess`, package re-pointed at this repo via `file:` while waiting on the OpenUPM publish): `Logs/Editor.log` shows the package re-resolving to `source: local`, then a full compile with **no `CS0246` and no `GUID […] conflicts with` line** after it; `Library/ScriptAssemblies/Zero.UI.dll` now contains the `PopupHandle` / `IPopupHandle` symbols, which is the direct proof the file made it into the AssetDatabase this time. **EditMode tests not run in that session** — the Unity MCP CLI was unavailable (plugin auto-bumped 0.81.1 → 0.86.1 mid-session and the new CLI requires an `ai-game.dev` login). Closed out on PR #11: `tests.yml` ran the suite headless on the `6000.5.5f1` image — **119 tests, 118 passed, 0 failed, 1 skipped** (`UITransitionInterruptionTests.SequentialTransitions_AfterCancellation_Works`, `[Ignore]`d in source). Tag `v0.5.2` + GitHub release for OpenUPM pickup.
