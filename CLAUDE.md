...
- `UIService` (`Zero.UI`, Phase 3) is the only impl of `IUIService`. Manages popup stack (generic `PushAsync<TPopup, TData, TResult>`), fullscreen screens, and toast queue. Layer canvases (Hud/Popup/Overlay/System, sort orders 100/200/300/400) are runtime-spawned by `UIStep` with `DontDestroyOnLoad`. No UI prefabs ship with the template; consumers provide popup/screen/toast prefabs via Addressables key conventions (`ui/popup/<name>`, `ui/screen/<name>`, `ui/toast/default`).
...
- **Layer canvases are runtime-spawned** (Phase 3). `UIStep` calls `LayerCanvas.Build()` which creates 4 Canvas GameObjects (`[Zero.UI.Hud]`, `[Zero.UI.Popup]`, `[Zero.UI.Overlay]`, `[Zero.UI.System]`) as children of a `[Zero.UI]` root, then marks them `DontDestroyOnLoad` (guarded by `Application.isPlaying`). No canvases are placed in `Bootstrap.unity`; they appear only after `UIStep` runs.
