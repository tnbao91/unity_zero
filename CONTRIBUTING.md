...
4. **Bootstrap step** (only if init must happen at startup) — `<Name>Step : BootstrapStepBase` at `Packages/com.tnbao91.nobody.zero/Runtime/Bootstrap/Steps/<Name>Step.cs`. Override `Name`, `IsCritical` (default false, and **no shipped step overrides it** — a critical failure aborts bootstrap onto a splash screen with no retry UI. Reach for it only in a consumer step that genuinely gates the game, and only if you ship the retry screen too), and optionally `Timeout` / `MaxRetries`.
